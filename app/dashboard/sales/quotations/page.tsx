...
export default function QuotationsPage() {
  ...
  const handleDelete = (record: Quotation) => {
    ...
    setQuotations(
      quotations.filter((quotation) => quotation.key !== record.key)
    );
  };
  ...
  const handleSend = (record: Quotation) => {
    message.success(
      `Quotation ${record.quotationNo} sent to ${record.customer}`
    );
  ...
  const columns: DynamicTableColumn<Quotation>[] = [
    ...
    },
    {
      ...
      render: (date: string, record: Quotation) => {
        const isExpired =
          new Date(date) < new Date() && record.status !== "accepted";
        return (
          ...
          <div>
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="Status">
                <Tag
                  icon={getStatusIcon(selectedQuotation.status)}
