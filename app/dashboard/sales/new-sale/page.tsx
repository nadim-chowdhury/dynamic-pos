...
const demoProducts: Product[] = [
  ...
  { id: '2', name: 'Mechanical Keyboard', price: 89.99, stock: 30, sku: 'KB-002' },
  { id: '3', name: 'USB-C Cable', price: 12.99, stock: 100, sku: 'CB-003' },
  { id: '4', name: 'Laptop Stand', price: 45.50, stock: 25, sku: 'LS-004' },
  { id: '5', name: 'Webcam HD', price: 65.00, stock: 15, sku: 'WC-005' },
  { id: '6', name: 'Headphones', price: 120.00, stock: 20, sku: 'HP-006' },
...
export default function NewSalePage() {
    ...
    if (existingItem) {
      const updatedItems = cartItems.map((item) =>
        item.productId === productId
          ? { ...item, quantity: item.quantity + 1, total: calculateItemTotal({ ...item, quantity: item.quantity + 1 }) }
          : item
      );
      ...
      const newItem: CartItem = {
        ...
        discount: 0,
        tax: 10,
        total: calculateItemTotal({ price: product.price, quantity: 1, discount: 0, tax: 10 }),
      };
  ...
  const handleSale = (isDraft: boolean = false) => {
    ...
    message.success(isDraft ? 'Sale saved as draft' : 'Sale completed successfully!');
  ...
  const productOptions = demoProducts
    ...
    .map((p) => ({
      value: p.id,
      label: `${p.name} (${p.sku}) - $${p.price.toFixed(2)} - Stock: ${p.stock}`,
    }));
    ...
    },
    {
      ...
      render: (quantity, record) => (
        <InputNumber
          min={1}
          value={quantity}
          onChange={(value) => updateCartItem(record.key, 'quantity', value || 1)}
          style={{ width: '100%' }}
        />
      ),
    },
    {
      ...
      render: (discount, record) => (
        <InputNumber
          ...
          max={100}
          value={discount}
          onChange={(value) => updateCartItem(record.key, 'discount', value || 0)}
          style={{ width: '100%' }}
          ...
          <Card title="Sale Items" style={{ marginBottom: 16 }}>
            <Space.Compact style={{ width: '100%', marginBottom: 16 }}>
              ...
              <Button
                type="primary"
                icon={<PlusOutlined />}
                onClick={() => selectedProduct && addProductToCart(selectedProduct)}
            ...
            <Row justify="end">
              <Col xs={24} sm={12} md={8}>
                <Space direction="vertical" style={{ width: '100%' }} size="small">
                  ...
                  <Row justify="space-between">
                    <Text>Discount:</Text>
                    <Text type="danger">-${totals.totalDiscount.toFixed(2)}</Text>
                  </Row>
                  <Row justify="space-between">
                    <Text>Tax:</Text>
