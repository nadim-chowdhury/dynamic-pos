...
export default function CategoriesPage() {
  ...
  const handleDelete = (record: Category) => {
    if (record.productCount > 0) {
      message.warning(`Cannot delete category "${record.name}" as it has ${record.productCount} products assigned`);
      return;
    }
  ...
  const CategoryForm = ({ onFinish }: { onFinish: (values: any) => void }) => (
