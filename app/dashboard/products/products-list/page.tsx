...
export default function ProductsListPage() {
  ...
  const columns: DynamicTableColumn<Product>[] = [
    ...
    },
    {
      ...
      render: (status: string) => (
        <Tag icon={status === 'active' ? <CheckCircleOutlined /> : <CloseCircleOutlined />} color={getStatusColor(status)}>
          {status.toUpperCase()}
        </Tag>
      ),
      filters: [
  ...
  const ProductForm = ({ onFinish }: { onFinish: (values: any) => void }) => (
    <Form form={form} layout="vertical" onFinish={onFinish}>
      ...
      <Form.Item>
        <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
          <Button onClick={() => { setIsCreateModalOpen(false); setIsEditModalOpen(false); form.resetFields(); }}>
            Cancel
          </Button>
  ...
  return (
    <div>
      <DynamicTable
        ...
        onAdd={() => { form.resetFields(); setIsCreateModalOpen(true); }}
        rowKey="key"
        scroll={{ x: 1400 }}
      />
      ...
      <DynamicModal
        ...
        width={800}
        footer={[
          <Button key="edit" type="primary" icon={<EditOutlined />} onClick={() => { setIsViewModalOpen(false); handleEdit(selectedProduct!); }}>
            Edit Product
          </Button>,
      ...
      >
        {selectedProduct && (
          <Descriptions bordered column={2}>
            ...
            <Descriptions.Item label="Product Name" span={2}>{selectedProduct.name}</Descriptions.Item>
            ...
            <Descriptions.Item label="Cost Price">${selectedProduct.cost.toFixed(2)}</Descriptions.Item>
            <Descriptions.Item label="Selling Price">${selectedProduct.price.toFixed(2)}</Descriptions.Item>
            <Descriptions.Item label="Stock Quantity">{selectedProduct.stock}</Descriptions.Item>
            <Descriptions.Item label="Minimum Stock">{selectedProduct.minStock}</Descriptions.Item>
            <Descriptions.Item label="Stock Status">
              <Tag color={getStockStatus(selectedProduct.stock, selectedProduct.minStock).color}>
                {getStockStatus(selectedProduct.stock, selectedProduct.minStock).text}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label="Status">
              <Tag icon={selectedProduct.status === 'active' ? <CheckCircleOutlined /> : <CloseCircleOutlined />} color={getStatusColor(selectedProduct.status)}>
      ...
      <DynamicModal
        ...
        onClose={() => { setIsCreateModalOpen(false); form.resetFields(); }}
        title="Add New Product"
        width={800}
        footer={null}
      >
        <ProductForm onFinish={handleCreateProduct} />
      </DynamicModal>

      {/* Edit Product Modal */}
      <DynamicModal
        isOpen={isEditModalOpen}
        onClose={() => { setIsEditModalOpen(false); form.resetFields(); }}
        title={`Edit Product - ${selectedProduct?.name}`}
        width={800}
        footer={null}
      >
        <ProductForm onFinish={handleUpdateProduct} />
      </DynamicModal>
    </div>
  );
}
