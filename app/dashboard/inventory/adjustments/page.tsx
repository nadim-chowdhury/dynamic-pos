...
const demoProducts = [
  ...
  {
    value: "HP-006",
    label: "HP-006 - Noise Cancelling Headphones (Stock: 20)",
  },
];
...
export default function AdjustmentsPage() {
  ...
  const columns: DynamicTableColumn<Adjustment>[] = [
    ...
    },
    {
      ...
      render: (qty: number, record: Adjustment) => (
        <Text
          strong
          style={{ color: record.type === "increase" ? "#52c41a" : "#ff4d4f" }}
        >
  ...
  const handleCreateAdjustment = (values: any) => {
    const newAdjustment: Adjustment = {
      key: String(Date.now()),
      adjustmentNo: `ADJ-2024-${String(adjustments.length + 1).padStart(
        3,
        "0"
      )}`,
      ...
      >
        {selectedAdjustment && (
          <Descriptions bordered column={2}>
            ...
            <Descriptions.Item label="Previous Stock">
              {selectedAdjustment.previousStock}
            </Descriptions.Item>
            <Descriptions.Item label="New Stock">
              {selectedAdjustment.newStock}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="Status">
              <Tag
                icon={getStatusIcon(selectedAdjustment.status)}
